...
export const calculateGrossProfit = (sales, purchaseCostByProduct = {}) => {
  return sales.reduce((profit, item) => {
    ...
    const productId = item.productId || item.product?._id?.toString?.() || item.product || '';
...
export const getDashboard = async (req, res, next) => {
  try {
    ...
    const [
      ...
      Product.find({ $expr: { $lte: ['$stock', '$minimumStock'] } }).select('name stock minimumStock').lean(),
      Sale.aggregate([
        { $match: { createdAt: { $gte: start, $lte: end }, status: { $ne: 'CANCELLED' } } },
        { $group: { _id: null, totalSales: { $sum: '$totalAmount' }, saleCount: { $sum: 1 } } },
      ]),
      Purchase.aggregate([
        { $match: { createdAt: { $gte: start, $lte: end }, status: { $ne: 'CANCELLED' } } },
        { $group: { _id: null, totalPurchases: { $sum: '$totalAmount' }, purchaseCount: { $sum: 1 } } },
      ]),
      Sale.aggregate([
      ...
      Sale.aggregate([
        { $match: { createdAt: { $gte: new Date(new Date().getFullYear(), 0, 1), $lte: new Date(new Date().getFullYear(), 11, 31, 23, 59, 59, 999) }, status: { $ne: 'CANCELLED' } } },
        { $group: { _id: { $dateToString: { format: '%Y-%m', date: '$createdAt' } }, totalSales: { $sum: '$totalAmount' }, orderCount: { $sum: 1 } } },
        { $sort: { _id: 1 } },
      ]),
      SaleItem.aggregate([
        { $lookup: { from: 'products', localField: 'product', foreignField: '_id', as: 'product' } },
        { $unwind: '$product' },
        { $group: { _id: '$product._id', productName: { $first: '$product.name' }, totalQuantity: { $sum: '$quantity' }, totalRevenue: { $sum: '$total' } } },
        { $sort: { totalRevenue: -1 } },
        { $limit: 5 },
      ]),
      SaleItem.aggregate([
        ...
        { $group: { _id: { $ifNull: ['$category.name', 'Uncategorized'] }, totalRevenue: { $sum: '$total' }, totalQuantity: { $sum: '$quantity' } } },
        { $sort: { totalRevenue: -1 } },
      ]),
      Purchase.aggregate([
        { $match: { createdAt: { $gte: start, $lte: end }, status: { $ne: 'CANCELLED' } } },
        { $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$createdAt' } }, totalPurchases: { $sum: '$totalAmount' }, orderCount: { $sum: 1 } } },
        { $sort: { _id: 1 } },
      ]),
      Purchase.aggregate([
        { $match: { status: { $ne: 'CANCELLED' } } },
        ...
        { $unwind: '$supplier' },
        { $group: { _id: '$supplier.name', totalPurchases: { $sum: '$totalAmount' }, orderCount: { $sum: 1 } } },
        { $sort: { totalPurchases: -1 } },
        { $limit: 10 },
      ]),
      Product.aggregate([
        { $group: { _id: null, totalUnits: { $sum: '$stock' }, inventoryValue: { $sum: { $multiply: ['$stock', '$costPrice'] } } } },
      ]),
    ]);

    const latestPurchaseCostByProduct = await PurchaseItem.aggregate([
      { $sort: { createdAt: -1 } },
      { $group: { _id: '$product', costPrice: { $first: '$costPrice' }, quantity: { $first: '$quantity' } } },
    ]);

    const purchaseCostMap = Object.fromEntries(latestPurchaseCostByProduct.map((entry) => [String(entry._id), Number(entry.costPrice || 0)]));
    ...
    const grossProfit = calculateGrossProfit(salesRevenueItems, purchaseCostMap);
    ...
    const inventoryTotals = inventoryValue[0] || { totalUnits: 0, inventoryValue: 0 };
    const outOfStockProducts = await Product.find({ stock: 0, isActive: true }).select('name sku stock minimumStock').lean();
